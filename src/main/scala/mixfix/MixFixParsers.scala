...
trait MixFixParsers extends StdTokenParsers with Operators with PackratParsers {
  ...
  def precedenceGraph: PrecedenceGraph
  ...
  class OpListParser(opGroup: OpList) {
    ...
    lazy val parser: PackratParser[OpTree] = closed | non | right | left

    lazy val tighter: PackratParser[OpTree] = parseOpGroups(precedenceGraph.higherPrecedence(opGroup), expectedTighter)

    lazy val closed: PackratParser[OpApply] = ⋁(closedOps map (_.parser), expectedClosed) ^^ {
      case op ~ internal => OpApply(op, names(internal), holes(internal))
    }
    
    lazy val non: PackratParser[OpApply] = 
      tighter ~ ⋁(nonOps map (_.parser), expectedNon) ~ tighter ^^ {
        case el ~ (op ~ internal) ~ er => OpApply(op, names(internal), el :: holes(internal) ::: List(er))
      }

    lazy val right: PackratParser[OpApply] = (
      ⋁(preOps map (_.parser), expectedRight) ^^ {
        case op ~ internal => OpApply(op, names(internal), holes(internal)) 
      } |
      tighter ~ ⋁(rightOps map (_.parser), expectedRight) ^^ {
    ...
    lazy val left: PackratParser[OpApply] = (left | tighter) ~ (
      ⋁(postOps map (_.parser), expectedLeft) ^^ {
        case op ~ internal => OpApply(op, names(internal), holes(internal)) 
      } |
      ⋁(leftOps map (_.parser), expectedLeft) ~ tighter ^^ {
    ...
    lazy val leftAssociative = tighter ~ (leftRest +) ^^ {
      case el ~ ops =>
        (el /: ops) {
    ...
    lazy val rightAssociative = (rightRest +) ~ tighter ^^ {
      case ops ~ er =>
        (ops :\ er) {
